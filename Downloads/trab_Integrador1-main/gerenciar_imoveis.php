...
// Verificar se o usuário está logado e é corretor ou admin
$auth->requireLogin('Login.php');
$current_user = $auth->getCurrentUser();

// Permitir acesso para corretores e administradores
...
<body id="page-top">
    ...
    <section class="contact-section bg-black" id="imoveis">
        <div class="container px-4 px-lg-5">
            ...
            <!-- Mensagens Flash -->
            <?php if ($flash): ?>
                <div class="alert alert-<?php echo isset($flash['type']) ? htmlspecialchars($flash['type']) : 'info'; ?> alert-dismissible fade show" role="alert">
                    <?php
                        $flashMessage = isset($flash['message']) ? $flash['message'] : '';
                        if (is_array($flashMessage)) {
                            $flashMessage = implode(', ', array_map(function($m){ return is_scalar($m) ? (string)$m : json_encode($m); }, $flashMessage));
                        }
                        echo htmlspecialchars((string)$flashMessage);
                    ?>
